/** @import { CsvMapping, ImportResult, ParsedCsv } from "../types.ts" */

/** @param {string} line */
const sniffDelimiter = (line) => {
  const counts = [",", ";", "\t"].map((delimiter) => ({ delimiter, hits: line.split(delimiter).length - 1 }));
  counts.sort((a, b) => b.hits - a.hits);
  return counts[0].hits > 0 ? counts[0].delimiter : ",";
};
...
export const guessMapping = (headers) => {
  ...
  return /** @type {CsvMapping} */ (mapping);
};
...
export const rowsToTransactions = (rows, mapping) => {
  /** @type {ImportResult["drafts"]} */
  const drafts = [];
  /** @type {ImportResult["skipped"]} */
  const skipped = [];
